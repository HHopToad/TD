...
bool MultiDirTower3::init()
{
	...
	tower= Sprite::createWithSpriteFrameName("tower3_3.png");
    this->addChild(tower);
    
    this->schedule(schedule_selector(MultiDirTower3::createBullet6), 0.8f);
	return true;
}


Sprite* MultiDirTower3::MultiDirTowerBullet()
{
	//add by  
...
void MultiDirTower3::createBullet6(float dt)
{
	...
	if(nearestEnemy != NULL && nearestEnemy->getCurrHp() > 0 )
    {
		for(int i = 0; i < dirTotal; i++)
		{
			...
			currBullet->runAction(Sequence::create(MoveTo::create(moveDuration, offscreenPoint),
												   CallFuncN::create(CC_CALLBACK_1(MultiDirTower3::removeBullet, this)),
												   NULL));
			currBullet = NULL;
		}
	}
...
void MultiDirTower3::removeBullet(Node* pSender)
{
    GameManager *instance = GameManager::getInstance();
    auto bulletVector = instance->bulletVector;
